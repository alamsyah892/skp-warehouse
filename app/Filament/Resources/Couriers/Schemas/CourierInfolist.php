...
class CourierInfolist
{
    ...
    {
        return Section::make('Courier Information')
            ->icon(Heroicon::Truck)
            ->iconColor('primary')
            ->description('Informasi utama dan identitas dasar Kurir.')
            // ->afterHeader([
            //     Action::make('edit')
            //         ->label('Edit')
    ...
    protected static function tabSection(): Tabs
    {
        return Tabs::make()
            ...
            ->tabs([
                ...
                //     ->schema([
                //         Callout::make()
                //             ->description('Riwayat semua Pengajuan Pembelian yang terkait dengan Kurir ini.')
                //             ->info()
                //             ->color(null)
                //         ,
                //     ])
    ...
    protected static function otherInfoSection(): Section
    {
        return Section::make('Other Information')
            ...
            ->iconColor('primary')
            ->description('Informasi lain terkait Kurir.')
            ->collapsible()
            ->columnSpanFull()
            ->columns(2)
            ->compact()
            ->schema([
                TextEntry::make('is_active')
                    ...
                    ->badge()
                    ->color(fn($state) => $state == Courier::STATUS_ACTIVE ? 'success' : 'warning')
                    ->columnSpanFull()
                ,
    ...
    protected static function relatedDataSection(): Section
    {
        return Section::make('Related Data')
            ...
            ->schema([
                Callout::make()
                    ->columnSpanFull()
                    ->description('Daftar entitas yang terhubung dengan Kurir ini.')
                    ->info()
                    ->color(null)
                ,
            ])
